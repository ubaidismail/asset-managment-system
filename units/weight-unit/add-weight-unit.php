...
if (isset($_POST['submit_unit_form'])) {
    ...
    $select = $conn->prepare('SELECT id FROM weight_units where name = ? or symbol = ?');
    ...
    if ($select->num_rows > 0) {
        ...
        $insert_query = $conn->prepare('INSERT INTO weight_units (name, symbol) values (?, ?)');
        $insert_query->bind_param('ss', $name, $symbol);
        if ($insert_query->execute()) {
            $success_mess = 'Weight Unit Inserted';
        } else {
    ...
    <div class="cat-add-form">
        ...
        <form method="POST" action="">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="unit_name" name="unit_name" placeholder="Kilogram, Gram, etc." required>
            </div>

            <div class="form-group">
                <label for="unit_symbol">Unit Symbol</label>
                <input type="text" class="form-control" id="unit_symbol" name="unit_symbol" placeholder="kg, g, etc." required>
            </div>
